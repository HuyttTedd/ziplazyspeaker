...
class Delete extends Action
{
    ...
    {
        $success = false;
        $dataDelete = $this->getRequest()->getParams();
        if(isset($dataDelete['delete_package'])) {
            try {
                foreach ($dataDelete['delete_package'] as $id) {
                    $this->packageFactory->create()->load($id)->delete();
                }
                $success = true;
            } catch (\Exception $e) {
        ...
        if(isset($dataDelete['delete_word'])) {
            try {
                foreach ($dataDelete['delete_word'] as $id) {
                    $this->wordFactory->create()->load($id)->delete();
                }
                $success = true;
            } catch (\Exception $e) {
                $dataCollection = [];
